...
def correction(filenames):
    ...
    from dir_wb import dir
    from dir_wb import stud_number
    ...
    #Generate IA Output
    wb_IA_output = openpyxl.load_workbook(path + '\\input\\IA_Output_empty_wb.xlsx', data_only=True) #load empty IA Output file
    ...
    ws_IA_output.cell(row=7+stud_number, column=3).value = first_name
    ws_IA_output.cell(row=7+stud_number, column=4).value = last_name
    ws_IA_output.cell(row=7+stud_number, column=5).value = points_stud
    ws_IA_output.cell(row=7+stud_number, column=6).value = olat_name
    if points_stud >= max_points*0.4: #passing limit
        ws_IA_output.cell(row=7+stud_number, column=7).value = '1' #means passed
    else: ws_IA_output.cell(row=7+stud_number, column=7).value = '0' #means failed
    ws_IA_output.cell(row=7+stud_number, column=9).value = points_stud_1_1
    ws_IA_output.cell(row=7+stud_number, column=10).value = points_stud_2_1
    ws_IA_output.cell(row=7+stud_number, column=11).value = points_stud_2_2
    ws_IA_output.cell(row=7+stud_number, column=12).value = points_stud_3_1
    ws_IA_output.cell(row=7+stud_number, column=13).value = points_stud_3_2
    ws_IA_output.cell(row=7+stud_number, column=14).value = points_stud_4
    ws_IA_output.cell(row=7+stud_number, column=15).value = points_stud_5
    wb_IA_output.save(path + '\\output\\IA_Output_wb.xlsx')
